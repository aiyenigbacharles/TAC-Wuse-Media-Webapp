...
export function DashboardOverview() {
  ...
  return (
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-balance text-foreground">Welcome back, {userName}!</h1>
        <p className="text-muted-foreground text-sm sm:text-base">Here's what's happening with your church media team today.</p>
      </div>
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-2 lg:grid-cols-4">
        <Card className="card-gradient border-border/50 hover:border-primary/20 transition-colors">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 sm:pb-2 p-3 sm:p-6">
            <CardTitle className="text-xs sm:text-sm font-medium text-foreground">Upcoming Services</CardTitle>
        ...
        <Card className="card-gradient border-border/50 hover:border-primary/20 transition-colors">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 sm:pb-2 p-3 sm:p-6">
            <CardTitle className="text-xs sm:text-sm font-medium text-foreground">Team Members</CardTitle>
        ...
        <Card className="card-gradient border-border/50 hover:border-primary/20 transition-colors">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 sm:pb-2 p-3 sm:p-6">
            <CardTitle className="text-xs sm:text-sm font-medium text-foreground">Media Files</CardTitle>
            <FileImage className="h-3 w-3 sm:h-4 sm:w-4 text-primary" />
          </CardHeader>
          <CardContent className="p-3 sm:p-6 pt-0">
        ...
        <Card className="card-gradient border-border/50 hover:border-primary/20 transition-colors">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 sm:pb-2 p-3 sm:p-6">
            <CardTitle className="text-xs sm:text-sm font-medium text-foreground">Hours Saved</CardTitle>
      ...
      <div className="grid gap-4 sm:gap-6 lg:grid-cols-2">
        ...
        <Card className="card-gradient border-border/50">
          <CardHeader>
            <CardTitle className="text-base sm:text-lg text-foreground">Your Upcoming Assignments</CardTitle>
            <CardDescription className="text-sm">Services you're scheduled for this week</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 sm:space-y-4">
            {upcomingServices.map((service) => (
              <div
                key={service.id}
                className="flex flex-col sm:flex-row sm:items-center justify-between p-3 border border-border/50 rounded-lg bg-card/30 hover:bg-card/50 transition-colors space-y-2 sm:space-y-0"
              >
                ...
                <Badge
                  ...
                  className={`${service.status === "confirmed" ? "bg-primary text-primary-foreground" : ""} text-xs`}
                >
                  {service.status}
                </Badge>
              </div>
            ...
            <Button
              variant="outline"
              className="w-full bg-transparent border-border hover:bg-card/50 hover:border-primary/30 mobile-btn"
            >
        ...
        <Card className="card-gradient border-border/50">
          <CardHeader>
            <CardTitle className="text-base sm:text-lg text-foreground">Team Members</CardTitle>
            <CardDescription className="text-sm">Active members in your media team</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 sm:space-y-4">
              ...
              >
                <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
                  <div className="relative">
                    ...
                    <div
                      className={`absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 sm:h-3 sm:w-3 rounded-full border-2 border-background ${
                        member.status === "online" ? "bg-green-500" : "bg-gray-400"
                      }`}
                    />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-xs sm:text-sm font-medium truncate text-foreground">{member.name}</p>
                    <Badge variant="outline" className={cn("text-xs glow-border", getRoleBadgeColor(member.role))}>
                      {member.role}
                    </Badge>
                  </div>
                </div>
                <Button variant="ghost" size="sm" className="hover:bg-card/50 p-1">
      ...
      <Card className="card-gradient border-border/50">
        <CardHeader>
          <CardTitle className="text-base sm:text-lg text-foreground">Recent Media</CardTitle>
          <CardDescription className="text-sm">Latest files uploaded to your media library</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            {recentMedia.map((media) => (
              ...
              >
                <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-medium text-foreground text-sm sm:text-base truncate">{media.name}</p>
                    <p className="text-xs sm:text-sm text-muted-foreground truncate">
                      {media.type} • {media.uploadedBy} • {media.date}
                    </p>
                  </div>
                </div>
                <Button variant="ghost" size="sm" className="hover:bg-card/50 p-1">
          ...
          <Button
            variant="outline"
            className="w-full mt-4 bg-transparent border-border hover:bg-card/50 hover:border-primary/30 mobile-btn"
          >
      ...
      <Card className="card-gradient border-primary/20 bg-gradient-to-r from-primary/5 to-primary/10">
        <CardContent className="p-6 sm:p-8 text-center">
          <div className="max-w-md mx-auto space-y-4">
            <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-primary/20 flex items-center justify-center mx-auto">
              <UserPlus className="h-5 w-5 sm:h-6 sm:w-6 text-primary" />
            </div>
            <h3 className="text-lg sm:text-xl font-bold text-foreground">Want to Join the Media Team?</h3>
            ...
            <Button className="bg-primary hover:bg-primary/90 text-primary-foreground font-medium px-6 sm:px-8 py-2 mobile-btn w-full sm:w-auto">
              Apply to Join Team
            </Button>
          </div>
        </CardContent>
      ...
      <div className="mt-6 sm:mt-8">
        <h2 className="text-xl sm:text-2xl font-bold text-foreground mb-4">Church Services Gallery</h2>
        <div className="relative overflow-hidden rounded-lg border border-border/50 bg-card/30">
          <div className="flex animate-scroll">
            <img
              ...
              alt="Worship Service"
              className="w-64 sm:w-80 h-32 sm:h-48 object-cover flex-shrink-0 mobile-img"
            />
            <img
